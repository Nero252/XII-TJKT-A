...
function renderMurid() {
    muridGrid.innerHTML = muridData.map((murid) => {
        ...
        const username = igUrl.replace('https://instagram.com/', '').replace('https://www.instagram.com/', '').replace('/', '');
        const isMobileDevice = isMobile();
        const targetUrl = isMobileDevice ? `instagram://user?username=${username}` : igUrl;
        ...
        return `
            <div class="murid-card">
                ...
                <div class="murid-name">${murid.nama}</div>
                <a href="${targetUrl}" onclick="handleInstagramClick(event, '${igUrl}')" class="murid-instagram">
                    <span class="murid-ig-btn">
                        <i class="fab fa-instagram"></i>
                        ${isMobileDevice ? 'Buka App' : 'Lihat IG'}
                    </span>
                </a>
            </div>
        `;
...
// SMART INSTAGRAM HANDLER
function handleInstagramClick(event, webUrl) {
    if (!isMobile()) {
        // LAPTOP: Buka web di tab baru
        window.open(webUrl, '_blank');
        return false;
    }
    
    // MOBILE: Coba app → fallback web
    ...
    muridGrid.innerHTML = muridData.map((murid) => {
        ...
        return `
            <div class="murid-card">
                ...
                <div class="murid-name">${murid.nama}</div>
                <a href="${appUrl}" onclick="tryInstagramApp(event, '${webUrl}')" class="murid-instagram">
                    <span class="murid-ig-btn">
                        <i class="fab fa-instagram"></i>
                        Instagram
                    </span>
...
// SMART INSTAGRAM APP LINK
function tryInstagramApp(event, webUrl) {
    event.preventDefault();
